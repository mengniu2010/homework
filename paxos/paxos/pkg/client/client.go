...
const (
	...
	RPCEcho   string = "Paxos.Echo"
	RPCNet    string = "Paxos.SetNet"
)
...
func (c *Client) SetMockNet(loss, delaySec int) (*mycommon.NetConfig, error) {
	if loss > 100 {
		return nil, fmt.Errorf("invalid arg")
	}
	arg := &mycommon.NetConfig{
		Loss:     loss,
		DelaySec: delaySec,
	}
	rep := &mycommon.NetConfig{
		Loss:     -1,
		DelaySec: -1,
	}
	err := c.client.Call(RPCNet, arg, rep)
	return rep, err
}

func (c *Client) GetMockNet() (*mycommon.NetConfig, error) {
